                ...
                case ZmqSocketOption.Identity:
                {
                    if (m_manual && m_lastPipe != null)
                    {
                        ...
                        if (optionValue is string)
                            ...
                            throw new InvalidException(string.Format("In XPub.XSetSocketOption(Identity, {0}) optionValue must be a string or byte-array.", optionValue != null ? optionValue.ToString() : "null"));
